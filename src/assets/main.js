...
function showAlert(type, title, message, options = {}) {
  ...
  if (actions.length > 0) {
    ...
    actions.forEach((action) => {
      ...
      const btnAction =
        action.action || `closeAlert(document.getElementById('${alertId}'))`;
...
function openStory(idx) {
  current = idx;
  renderStory();
  document.getElementById("storyBackdrop").classList.add("show");
  document.body.style.overflow = "hidden";
}
...
function renderStory() {
  ...
  document.getElementById("sAvatar").src = s.avatar;
  document.getElementById("sName").textContent = s.name;
  document.getElementById("sTime").textContent = s.time;

  const body = document.getElementById("storyBody");
  if (s.type === "text") {
    body.innerHTML = `<h2>${s.title}</h2><p>${s.body}</p>`;
  } else if (s.type === "button") {
    body.innerHTML = `<h2>${s.title}</h2><p>${s.body}</p><button class="story-cta" onclick="closeStory()">Do Something</button>`;
  } else if (s.type === "fixedbtn") {
    body.innerHTML = `<button class="story-fixed-btn" onclick="closeStory()">Fixed Button</button>`;
  } else {
    body.innerHTML = "";
  }
  ...
  stories.forEach((_, i) => {
    ...
    bar.className =
      "prog-bar" + (i < current ? " done" : i === current ? " active" : "");
...
function closeStory() {
  clearTimeout(timer);
  document.getElementById("storyBackdrop").classList.remove("show");
  document.body.style.overflow = "";
}
...
document.addEventListener("DOMContentLoaded", function () {
  const triggers = document.querySelectorAll(".nested-trigger");
  ...
  triggers.forEach((trigger) => {
    trigger.addEventListener("click", function (e) {
      ...
      // Toggle Class Active pada pemicu (untuk putar chevron)
      this.classList.toggle("active");

      // Toggle Class Show pada konten (untuk buka/tutup)
      if (targetEl) {
...
function showToastValidation() {
  const t = document.getElementById("toast-validation");
...
function hideFeedbackMD(id) {
  const el = document.getElementById(id);
  const field = el.closest(".md-field, .md-check-group");
  if (field) {
    field
...
function validateMD(form, e) {
  ...
  form.querySelectorAll("[required]").forEach((el) => {
    ...
    if (vEl) vEl.style.display = ok ? "block" : "none";
    if (iEl) iEl.style.display = ok ? "none" : "block";
...
function iUpdateUI() {
  for (let i = 0; i < I_TOTAL; i++) {
    // content panels
    const p = document.getElementById("i-sc-" + i + "p");
    if (p) p.classList.toggle("active", i === iStep);
    // step circles
    const sw = document.getElementById("i-sw-" + i);
    if (sw) {
      sw.classList.remove("done", "active");
      if (i < iStep) sw.classList.add("done");
      else if (i === iStep) sw.classList.add("active");
    }
    // connector lines
    if (i < I_TOTAL - 1) {
      const sl = document.getElementById("i-sl-" + i);
      if (sl) sl.classList.toggle("done", i < iStep);
    }
  }
  // prev
  const prevBtn = document.getElementById("i-prev-btn");
  prevBtn.style.display = iStep === 0 ? "none" : "block";
  // next
  const nextBtn = document.getElementById("i-next-btn");
  const nextLbl = document.getElementById("i-next-label");
  const nextIcon = document.getElementById("i-next-icon");
  if (iStep === I_TOTAL - 1) {
    nextBtn.classList.add("success");
    nextLbl.textContent = "New Order";
    nextIcon.setAttribute("name", "refresh-outline");
  } else if (iStep === I_TOTAL - 2) {
    nextBtn.classList.remove("success");
    nextLbl.textContent = "Pay Now";
    nextIcon.setAttribute("name", "card-outline");
  } else {
    nextBtn.classList.remove("success");
